...
for(i in 1:ncol(df_pl)){
  df_pl[is.na(df_pl[,i]), i] <- mean(df_pl[,i], na.rm = TRUE)
...
fit <- knnreg(train_data[,c(17,5)],train_data[,18],k=2)
predictions <- predict(fit, test_data[,c(17,5)])
...
i <- 1
for (k in seq(1, 39, 2)){
  fit_k <- knnreg(train_data[,c(5,11)],train_data[,18],k=k)
  pred_k <- predict(fit_k, test_data[,c(5,11)])
